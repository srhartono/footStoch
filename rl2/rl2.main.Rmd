...
source('lib/rl2.lib9.R')
source('lib/rl2.lib1.R')
source('../lib/tri.lib.misc.R')

B = file.parse_deltaG()
Bprint = subset(B[B$type == 'RD.min.DD',],select=c(chunk,B))
par = list(
  a = 10,
  π = pi,
  C = 1.8,
...
par$K = (2200 * par$R * par$T) / par$N
par$n = seq(1,3000)
par$len = seq(0,1000)
par$sigmas = c(0,-0.07,-0.14)

# seqgood = join(rep('G',100))
# seqbad  = join(rep('T',100))

```
"Whats the ΔG (energy*) to form Rloop with <len> length at <n> bp of a 2000bp long G-Homopolymer which has <sigma> negative supercoiling?"

*lower ΔG = easier to form Rloop

* a   = junction energy (10)
* n   = Rloop start position
* len = Rloop length

ΔG(n,len) = a + (2π^2 * C * K) / (4π^2 * C + K * len) * (alpha + len * A)^2 + sum(B[n:n+len])

```{r}
cat('\nParameters\n',
paste('- ',paste(names(par),':',par),collapse='\n'),
'\n-  alpha = N x sigma x A\n\n',
'\n-  B     = bp.energy = difference between energy to form RNA:DNA and DNA:DNA 
Example: rGG:dCC vs dGG:dCC ([top strand rGG, bot sstrand dCC] - [top strand dGG, bot strand dCC])
- bp.energy(rGG:dCC) = -2.2
- bp.energy(dGG:dCC) = -1.84
- Δbp.energy(rGG:dCC,dGG:dCC) = -2.2 - (-1.84) = -0.36 (negative -> for GG, it"s more likely to form RNA:DNA than DNA:DNA)
','
4 variables: n, len, sigma, my.sequence

ΔG(n,len) = 10 + (32.08 / (71.06 + 0.90 * len) * ((144.23 * sigma) + (0.096 * len)^2) + sum(B[n:n+len])',sep='')
```
...
Btable = read.table(BtableFile,sep="")
\nBtable:\nNuc| B(bp energy)\n--------------\n',

paste(Bprint$chunk[1:15]," | ",Bprint$B[1:15],sep="",collapse="\n"),

'\n(...etc)\n

1. create table of bp.energy for my.sequence (Bfinal)

calc.B2 = function(my.sequence) {
  # my.sequence is AAAAACCCCCTTTTTGGGGG
  
  # create dinucleotide table
  # i |Nuc
  # --------
  # 1  AA
  # 2  AA
  ...
  my.seq.df      = data.frame(seq(1,nchar(my.sequence)-1),
                              a=strsplit(my.sequence,"")[[1]][-nchar(my.sequence)],
                              b=strsplit(my.sequence,"")[[1]][-1]
                              )
  my.seq.df$Nuc  = paste(my.seq.df[,1],my.seq.df[,2],sep="")
  Bfinal = subset(merge(my.seq.df,Btable,by="Nuc"),select=c(index,Nuc,B))
  Bfinal = Bfinal[order(Bfinal$index),]
  return(Bfinal)
}


3. Calculate dG at n=n, len=len
calc.dG = function(n,len,sigma,Bfinal) {
  dG = 10 + (32.08 / (71.06 + 0.90 * len) * ((144.23 * sigma) + (0.096 * len)^2)) + sum(Bfinal$B[max(1,n):min(n+len,dim(Bfinal)[1])])
  return(dG)
}
...
"Whats the energy to form Rloop with 1000bp length at 1st bp of a 2000bp long G-Homopolymer which has sigma of -0.07?"

n     = 1 (rloop position)
len   = 200 (rloop length)
sigma = -0.07 (negative supercoiling)

ΔG(n,len) = 10 + (32.08 / (71.06 + 0.90 * len) * ((144.23 * sigma) + (len * 0.096)^2) + sum(bp.energy[n:n+len])


#################################
1. Seq Good (2kbp G homopolymer GGGGG...) -> bp.energy GG = -0.36


ΔG(1,200) = 10 + (32.08 / (71.06 + 0.90 * 200) * (144.23 * -0.07 + 0.096 * 200)^2) + sum(bp.energy[n:n+len])
ΔG(1,200) = 10 + 10.59                                                             + 200 * bp.energy(GG)
ΔG(1,200) = 10 + 10.59                                                             + 200 * -0.36
ΔG(1,200) = 10 + 10.59                                                          +    -72 
ΔG(1,200) = -51.4 (likely)




#################################
2. Seq Bad (2kbp T homopolymer) -> bp.energy TT = 0.8

ΔG(1,200) = 10 + (32.08 / (71.06 + 0.90 * 200) * (144.23 * -0.07 + 0.096 * 200)^2) + 200 * 0.8
ΔG(1,200) = 10 + 10.59                                                             + 160
ΔG(1,200) = 180.59 (unlikely)

#################################
3. Seq Ok (2kbp 85% GC, 0 GC Skew)

ΔG(1,200) = 10 + 10.59 + 41.29
ΔG(1,200) = 61.88 (more likely)


#################################
4. Seq Good (2kbp 85% GC, 0.5 GC Skew)

ΔG(1,200) = 10 + 10.59 + -4.1
ΔG(1,200) = 16.49 (even more likely)


To get probability of R-loop with len of 0 to 1000, at n position 1 to N in that sequence:

sum1 = 0
sum2 = 0
for (i in 1:1000) {
  for (j in 1:1000) {
    sum1 = exp(-1*ΔG(n=i,len=j)/(R*T))/
    sum2 = sum2 + sum(exp(-1*ΔG(n=i,len=j)/(R*T))))
...
# FASTAS.rl2 = BIGFASTAS
# FASTAS.rl2[grep('pFC53',FASTAS.rl2$gene),]$seq = sapply(FASTAS.rl2[grep('pFC53',FASTAS.rl2$gene),]$seq,revcomp)
# FASTAS.rl2[grep('pFC53',FASTAS.rl2$gene),]$seq.amp = sapply(FASTAS.rl2[grep('pFC53',FASTAS.rl2$gene),]$seq.amp,revcomp)
# FASTAS.rl2$type = 'invivo'
# FASTAS.rl2[grep('pFC|T7',FASTAS.rl2$gene,ignore.case = TRUE),]$type = 'invitro'
# FASTAS.rl2 = FASTAS.rl2[order(FASTAS.rl2$type,FASTAS.rl2$gene),]
# saveRDS(FASTAS.rl2,'resources/FASTAS.rl2.RDS')
FASTAS.rl2 = readRDS('resources/FASTAS.rl2.RDS')
seqT = seqr$seq$seqT
seqG = seqr$seq$seqG
seqgood = seqr$seq$seqgood
seqok = seqr$seq$seqok
# seqVR10 = FASTAS.rl2[FASTAS.rl2$chr == 'T7_init_VR_10',]$seq
# seqVR20 = FASTAS.rl2[FASTAS.rl2$chr == 'T7_init_VR_20',]$seq
#seqtypes = c(FASTAS.rl2$gene[c(seq(1,32),65,66,73,80,84,88,93,64,seq(33,63))])
#seqtypes = c(seqtypes,'seqT','seqG','seqgood','seqok','T7_init_VR_10','T7_init_VR_20')
#myseqs =   c(FASTAS.rl2$seq[ c(seq(1,32),65,66,73,80,84,88,93,64,seq(33,63))])
#myseqs =   c(myseqs,seqT,seqG,seqgood,seqok,seqVR10,seqVR20)
len.max = max(par$len)
seqtypeswant = c("pFC9","T7_init_VR_10","T7_init_VR_20","CALM3","FUS","pFC53_ApaLI","pFC53TVR_10_pair_0_T3TermMix","pFC53TVR_20_pair_0_T3TermMix")

FASTASwant = FASTAS.rl2[FASTAS.rl2$gene %in% seqtypeswant,]
circ = pasta(FASTASwant$seq,substr(FASTASwant$seq,1,len.max))
myseqs = c(FASTASwant$seq,circ,seqT,seqG,seqgood,seqok)
seqtypes = c(FASTASwant$gene,pasta(FASTASwant$gene,'_circ'),"seqT","seqG","seqgood","seqok")
...
for (i in 1:length(myseqs)) {
  ...
  if (file.exists(file.dfseq)) {
    ...
    dfseq1 = rl2.fa.get_chunk(seq1,window=par$window);size(dfseq1);
    #  dfseq1$i0 = dfseq1$i0 #+ par$window/2
    rownames(dfseq1) = dfseq1$i0
    saveRDS(dfseq1,file=file.dfseq)
    print('  -> save')
  }  
  print(paste('- dg.merge',seqtype))
  if (file.exists(file.dgseq)) {
    dgseq1 = readRDS(file.dgseq)
  } else {
    dgseq1 = subset(dg.merge(B,seq1),select=c(i0,DD,RD,RD.min.DD));size(dgseq1)
    rownames(dgseq) = dgseq$i0
    saveRDS(dgseq1,file=file.dgseq)
    print('  -> save')
  }
  # next
  #file = pasta('resources/',seqtype,'_',par$window,'_.RDS')
  # if (file.exists(file)) {
  # #   load(file)
  #   dgall1$seqtype = seqtype
  #   dgall1$fileInd = i
  ...
  #   dg.all = rbind(dg.all,dgall1)
  #   print(paste('- ',file,'exist!'))
  #   next
  # } else {
  #   print(paste('- ',file,'doesnt exist!!!'))
  # }
  
  
  print(paste('- dg.calc',seqtype))
  if (file.exists(file.dgall)) {
    dgall1 = readRDS(file.dgall)
    dgall1$seqtype = seqtype
    dgall1$fileInd = i
    dfseq1$seqtype = seqtype
    dfseq1$fileInd = i
    ...
    next
    seq.len.max = seq(1,nchar(seq1) - len.max)
    mygrid = as.data.frame(expand.grid(n=seq.len.max,m=par$len))
    mygrid = mygrid[order(mygrid$n,mygrid$m),]
    mygrid$RD.min.DD = 0
    #dgseq2 = dgseq$RD.min.DD
    #rownames(dgseq2) = dgseq$i0
    mygrid$RD.min.DD = apply(mygrid,1,dgb,dgseq=dgseq1$RD.min.DD)
    dgall1 = cbind(B.sigma,subset(mygrid,select=-m))
    #dgall1 = apply(seq.len.max,dg.calc,m.all=par$len,dgseq=dgseq1,dg=B.sigma,seqtype=seqtype)
    #dgall2 = dg.calclong(seq.len.max,par$len,dgseq1,B.sigma,seqtype=seqtype)
    dgall1$dgmin00 = dgall1$'0' + dgall1$RD.min.DD
    dgall1$dgmin07 = dgall1$'-0.07' + dgall1$RD.min.DD
    dgall1$dgmin14 = dgall1$'-0.14' + dgall1$RD.min.DD
    print('  -> save')
    saveRDS(dgall1,file=file.dgall)
  }
}

seqwant = c('T7_init_VR_10','T7_init_VR_20','pFC9',
            'T7_init_VR_10_circ','T7_init_VR_20_circ','pFC9_circ')
